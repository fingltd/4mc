 ...
 * files in 4mz format, seamlessly handling the input split that leverages the internal 4mz block index.
 * Usage:
 * where you usually set input:
 * MultiInputFormat.setInputFormatClass(MyProtoMessage.class, job);
 * replace with:
 * FourMzEbProtoInputFormat.setInputFormatClass(MyProtoMessage.class, job);
 * That's it.
 * <p>
 * Output format doesn't need anything, as if you want to write text output and compress it,
 * you can just do the usual:
 * XyzOutputFormat.setCompressOutput(job, true);
 * XyzOutputFormat.setOutputCompressorClass(job, FourMzCodec.class);  // or e.g. FourMzMediumCodec.class
 * <p>
 * If instead you need to write binary protobuf as well, see FourMzEbProtoOutputFormat.
 *
 * @param <M> protobuf message to be read.
 */
public class FourMzEbProtoInputFormat<M extends Message> extends FourMzInputFormat<LongWritable, BinaryWritable<M>> {

    private TypeRef<M> typeRef;
    private static String CLASS_CONF_KEY = "elephantbird.class.for.FourMzEbProtoInputFormat";

    public static void setInputFormatClass(Class<?> clazz, Job job) {
        job.setInputFormatClass(FourMzEbProtoInputFormat.class);
        setClassConf(clazz, job.getConfiguration());
    }

    public FourMzEbProtoInputFormat() {
    }

    public FourMzEbProtoInputFormat(TypeRef<M> tr) {
        this.typeRef = tr;
    }
